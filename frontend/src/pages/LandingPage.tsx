...
const regionalSelections = [
  ...
  { id: 2, name: "Santorini", country: "Greece", img: "https://images.unsplash.com/photo-1570077188670-e3a8d69ac5ff?w=300&q=80" },
  { id: 3, name: "Kyoto", country: "Japan", img: "https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?w=300&q=80" },
  { id: 4, name: "Machu Picchu", country: "Peru", img: "https://images.unsplash.com/photo-1526392060635-9d6019884377?w=300&q=80" },
...
const Banner: React.FC = () => (
  <div
    ...
    style={{ height: "240px", boxShadow: "0 8px 32px rgba(198,93,58,0.2)" }}
  >
    <img
    ...
    <div className="absolute inset-0"
      style={{ background: "linear-gradient(to top, rgba(28,22,18,0.82) 0%, rgba(198,93,58,0.2) 55%, transparent 100%)" }}
    />
    <div className="relative z-10 px-6 pb-6">
      <p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: "#E6D3B3" }}>
        Discover the world
      </p>
...
const SearchBar: React.FC<{ dark: boolean }> = ({ dark }) => {
  const [query, setQuery] = useState("");
  const btnStyle = {
    background: dark ? "rgba(42,33,26,0.85)" : "rgba(243,233,220,0.75)",
    backdropFilter: "blur(12px)",
    WebkitBackdropFilter: "blur(12px)",
    border: dark ? "1px solid rgba(61,46,34,0.8)" : "1px solid rgba(230,211,179,0.8)",
    ...
    <div className="mx-4 mt-4 flex items-center gap-2">
      <div className="flex-1 relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 transition"
          style={btnStyle}
        />
      </div>
...
const SectionHeader: React.FC<{ title: string; dark: boolean }> = ({ title, dark }) => (
  <div className="flex items-center gap-3 mx-4 mt-7 mb-3 w-full">
    ...
    <hr
      ...
      style={{
        background: dark
          ? "linear-gradient(to right, rgba(212,163,115,0.6), rgba(212,163,115,0.1))"
          : "linear-gradient(to right, rgba(59,47,47,0.25), rgba(59,47,47,0.05))",
        minWidth: "40px",
      }}
    />
  </div>
...
const RegionalCard: React.FC<{ name: string; country: string; img: string }> = ({ name, country, img }) => (
  <button
    className="flex-shrink-0 rounded-2xl overflow-hidden relative hover:scale-105 transition-transform focus:outline-none"
    style={{
      width: "128px", height: "128px",
      boxShadow: "0 4px 16px rgba(28,22,18,0.25)",
      border: "1px solid rgba(255,255,255,0.15)",
    }}
  >
    <img src={img} alt={name} className="absolute inset-0 w-full h-full object-cover" />
    <div className="absolute inset-0"
      style={{ background: "linear-gradient(to top, rgba(28,22,18,0.75) 0%, transparent 55%)" }} />
...
const LandingPage: React.FC = () => {
  ...
  return (
    <div
      className="min-h-screen pb-28 relative transition-colors duration-300"
